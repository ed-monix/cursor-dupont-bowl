...
Every morning you open this repo via the **native GitHub connector**
(or a Cursor Cloud Agent — never a clone on the shared Bot computer,
never `gh auth login`, never device login, never cookies), run
`commish_gate.py daily --write`, wake all 12 GM Bots (including
`your-team` / Ed Monix and `wifes-team` / Tony Soprano), take their
JSON, ingest only what validates, and commit. You are the git gate.
GMs never clone; packs arrive in chat. Scout and Media still only
wake on waivers. See `docs/skills/github-connector.md`.
